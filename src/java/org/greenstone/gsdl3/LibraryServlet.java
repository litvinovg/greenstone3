...
* takes the a=p&p=home type args and builds a simple request to send to
* its receptionist, which returns a result in html, cos output=html
* is set in the request
* 
* 18/Jul/07 xiao
* modify to make the cached parameters collection-specific.
* Most of the work is done in doGet(), except adding an inner class UserSessionCache.
* 
* @see Receptionist
*/
...
* its used by the servlet to determine what args to save */
	...
	/** user id - new one per session. This doesn't work if session state is saved between restarts - this requires this value to be saved too. */
	protected int next_user_id = 0;

	/** a hash that contains all the active session IDs mapped to the cached items
*  It is updated whenever the whole site or a particular collection is reconfigured
*  using the command a=s&sa=c or a=s&sa=c&c=xxx
*  It is in the form: sid -> (UserSessionCache object)
*/
	protected Hashtable session_ids_table = new Hashtable();

	/** the maximum interval that the cached info remains in session_ids_table (in seconds) 
...
*/
	...
	public class UserSessionCache implements HttpSessionBindingListener {
		...
		/** a hash that maps the session ID to a hashtable that maps the coll_name to its parameters 
	*  coll_name -> Hashtable (param_name -> param_value)
	*/
	...
	throws ServletException, IOException {
		...
		if (query_string!=null){
			...
			if ((redirect) && (href != null) && (rl.equals("0"))){// This is an external link, the web page is re-directed to the external URL (&el=&rl=0&href="http://...")
		...
		while(attributeNames.hasMoreElements()) {
			String name = (String)attributeNames.nextElement();
			if (!name.equals (GSXML.USER_SESSION_CACHE_ATT)
					&& !name.equals (GSParams.LANGUAGE) 
					&& !name.equals (GSXML.USER_ID_ATT)) {
	...
	/** this goes through each URL and adds in a session id if needed--
* its needed if the browser doesn't accept cookies
* also escapes things if needed
*/
