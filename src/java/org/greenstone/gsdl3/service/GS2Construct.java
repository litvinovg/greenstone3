...
/**
 * A Services class for building collections
 * provides a wrapper around the old perl scripts
 *
...
public class GS2Construct 
	...
	if (GSStatus.isCompleted(status_code) && 
	    GSStatus.isError(status_code)) {
    ...
    /** send a configure request to the message router 
     * action name should be "delete" or "reload"
     * response will be put into the status element
     */
     ...
     * for now, all services have type=build - need to think about this
     */
	...
	if (get_status_only) {
		...
		if (listener.isFinished()) { // remove this listener - its job is done
	...
	if (!coll_dir.exists()) { // this name is ok - not used yet
	    return base_name;
	}
    ...
    /** takes the params from the request (in the HashMap) and extracts any
     * that need to be passed to the constructor and puts them into a 
     * paramList element */
