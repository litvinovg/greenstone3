    ...
    extends AbstractGS2Search 
{
   ...
   // the default level for searching
    ...
    abstract protected boolean addTermInfo(Element term_list, HashMap params,
					   Object query_result);
    
    /** combines all the field params into a single query
     * - for simple field query */
	...
	String combine = "";
	for (int i=0; i<texts.length; i++) {
	    if (i==0) {// assume first one is blank
		combine = "";
	    } else {
		String x = combines[i]; 
    ...
    abstract protected void addQueryElem(StringBuffer final_query, 
					 String query, String field, 
					 String combine);
    abstract protected String addStemOptions(String query, String stem,
