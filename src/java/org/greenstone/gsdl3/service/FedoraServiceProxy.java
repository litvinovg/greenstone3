 ...
 */
public class FedoraServiceProxy extends ServiceRack implements OID.OIDTranslatable
{
	 ...
	 * 
	 * @param info the XML node <serviceRack name="XXX"/> with name equal to the
	 ...
	 */
	/*
	 * public boolean configure(Element info) { return configure(info, null); }
	 */
	 ...
	 * "DocumentStructureRetrieve", "TextQuery", "FieldQuery",
	...
	protected Element processClassifierBrowseMetadataRetrieve(Element request)
	{
		...
		Element response = getResponseAsDOM(fedoraServicesAPIA.retrieveBrowseMetadata(this.cluster_name, "ClassifierBrowseMetadataRetrieve", classIDs, metafields));
		return (Element) response.getElementsByTagName(GSXML.RESPONSE_ELEM).item(0);
	}
	...
	protected Element processClassifierBrowse(Element request)
	{
		...
		Element response = getResponseAsDOM(fedoraServicesAPIA.retrieveBrowseStructure(collection, "ClassifierBrowse", classifierIDs, new String[] { structure }, new String[] { info }));
	...
	/**
	 * if id ends in .fc, .pc etc, then translate it to the correct id For now
	 * (for testing things work) the default implementation is to just remove
	 * the suffix
	 */
	...
	/**
	 * translates relative oids into proper oids: .pr (parent), .rt (root) .fc
	...
	public String processOID(String doc_id, String top, String suff, int sibling_num)
	{
		...
		if (doc_id.startsWith("CL"))
		{ // classifiernode
			response = getResponseAsDOM(fedoraServicesAPIA.retrieveBrowseStructure(this.cluster_name, "ClassifierBrowse", new String[] { doc_id }, new String[] { "children" }, new String[] { "siblingPosition" }));
		...
		{ // documentnode
			response = getResponseAsDOM(fedoraServicesAPIA.retrieveDocumentStructure(this.cluster_name, new String[] { doc_id }, new String[] { "children" }, new String[] { "siblingPosition" }));
			String path = GSPath.createPath(new String[] { GSXML.RESPONSE_ELEM, GSXML.DOC_NODE_ELEM + GSXML.LIST_MODIFIER, GSXML.DOC_NODE_ELEM, GSXML.NODE_STRUCTURE_ELEM, GSXML.DOC_NODE_ELEM });
	...
	{
		if (response == null)
		{ // will not be the case, because an empty  
	...
	/*
	 * //process method for stylesheet requests protected Element
	 * processFormat(Element request) {}
	 */

	/* returns the service list for the subclass */
	/*
	 * protected Element getServiceList(String lang) { // for now, it is static
	 * and has no language stuff return (Element)
	 * this.short_service_info.cloneNode(true); }
	 */
