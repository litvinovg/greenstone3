...
public class NoCollQueryAction extends Action {
    ...
    /** a generic query handler 
     * this gets the service description, does the query (just passes all teh 
     * params to the service, then gets the titles for any results 
     */
	...
	if (service_params == null) { // no query
	    return page_response;
	}
	...
	Element mr_meta_message = this.doc.createElement(GSXML.MESSAGE_ELEM);
	...
	// the result 
	Element result_doc_list = this.doc.createElement(GSXML.DOC_NODE_ELEM+GSXML.LIST_MODIFIER);
	...
	Element coll_param_list = this.doc.createElement(GSXML.PARAM_ELEM+GSXML.LIST_MODIFIER);
