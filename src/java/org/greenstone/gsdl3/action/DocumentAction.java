...
public class DocumentAction extends Action {
   ...
   static Logger logger = Logger.getLogger(org.greenstone.gsdl3.action.DocumentAction.class.getName());
    ...
    /** if this is set to true, when a document is displayed, any annotation
     * type services (enrich) will be offered to the user as well */
	...
	if (expand_document) { // we always expand the contents with the text
	...
	} else { // a simple type - we dont have a dummy node for simple
	...
	for (int i = 0; i < doc_nodes.getLength(); i++) {
	    ...
	    dm_doc_node.setAttribute(GSXML.NODE_TYPE_ATT,
				     doc_node.getAttribute(GSXML.NODE_TYPE_ATT));
	}
     ...
     * if an action has its own arguments, this should add them to the params
     * object - particularly important for args that should not be saved */
    ...
    /** this method gets the collection description, the format info, the
     * list of enrich services, etc - stuff that is needed for the page,
     * but is the same whatever the query is - should be cached */
    protected  boolean getBackgroundData(Element page_response, 
					 String collection, String lang,
					 String uid) {
	...
	if (service_name == null || !service_name.endsWith("Query")) { // hack for now - we only do highlighting if we were in a query last - ie not if we were in a browse thingy
