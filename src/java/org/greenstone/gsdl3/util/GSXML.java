...
public class GSXML {
  ...
  public static HashMap extractParams(Element xml, boolean deep, String toFind) {
    ...
    if (deep) { // get all the nested ones
      params = xml.getElementsByTagName(PARAM_ELEM);
    } else { // just get the top  level ones
  ...
  /** add an error message */
  public static boolean addError(Document owner, Element doc,
    Throwable error, String error_type) {
  ...
  public static boolean addMetadata(Document owner, Element list,
    String meta_name, String meta_value) {
  ...
  /** takes an xpath type expression of the form name/name/...
   * and returns the first node that matches, or null if not found */
  ...
  /** takes an xpath type expression of the form name/name/...
   * and returns the first node that matches, or null if not found
   * can include [i] indices. index numbers start at 0 */
  ...
  public static Element duplicateWithNewName(Document owner, Element element,
    String element_name, boolean with_attributes) {
    return duplicateWithNewNameNS(owner, element, element_name, null, with_attributes);
  }
  
  /** Duplicates an element, but gives it a new name */
  public static Element duplicateWithNewNameNS(Document owner,
    Element element,
    String element_name,
  ...
  public static  Element createBasicRequest(Document owner,
    String request_type, String to,
    String lang,
    String uid) {
  ...
  public static Element createParameter(Document owner, String name,
    String value) {
  ...
  public static Element createParameterDescription(Document owner,
    String id,
    String display_name,
    String type,
  ...
  public static Element createParameterDescription2(Document owner,
    String id,
    String display_name,
    String type,
    String default_value,
    ArrayList option_ids,
    ArrayList option_names) {
  ...
  /** returns a NodeList of elements: ancestor/node_name[@attribute_name='attribute_value']
   */
  ...
  // sort type:
  public static Element insertIntoOrderedList(Element parent_node,
    String node_name,
    Element start_from_elem,
    Element new_elem, String sort_att,
    boolean descending) {
    ...
    while ((!descending && list_att.compareTo(insert_att)<0) || (descending && list_att.compareTo(insert_att)>0)) {
      ...
      if (current_node == null) break; // end of the list
  ...
  /** Returns the appropriate language element from a display elem,
   * display is the containing element, name is the name of the element to
   * look for, lang is the preferred language, lang_default is the fall back
   * lang if neither lang is found, will return the first one it finds*/
  public static String getDisplayText(Element display, String name,
