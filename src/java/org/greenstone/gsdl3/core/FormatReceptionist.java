 ...
 */
public class FormatReceptionist extends TransformingReceptionist {
    ...
    protected void addExtraInfo(Element page) {
        ...
        info_message = this.doc.createElement(GSXML.MESSAGE_ELEM);
        String address = GSPath.appendLink(coll_name, "CoverageMetadataRetrieve");
        Element info_request = GSXML.createBasicRequest(this.doc,  GSXML.REQUEST_TYPE_PROCESS, address, lang, uid);
        Element req_param_list = this.doc.createElement(GSXML.PARAM_ELEM+GSXML.LIST_MODIFIER);
        req_param_list.appendChild(GSXML.createParameter(this.doc, "metadata", "All")); // GSXML.SUBSET_PARAM, GSXML.DISPLAY_TEXT_ELEM+GSXML.LIST_MODIFIER));
        info_request.appendChild(req_param_list);
        info_message.appendChild(info_request);

        logger.error("************ Coverage Metadata request created");
        logger.error("************ add extra info, page request="+this.converter.getString(info_message));

        info_response = (Element)this.mr.process(info_message);
        logger.error("************ page response="+this.converter.getString(info_response));
