...
function navigate(e){
	...
	if (target.id.toLowerCase() == 'fullview'){
		documentCheckoutHighlight( 'fullview' );
		showFullView();
	}

	if (target.id.toLowerCase() == 'textview'){
		documentCheckoutHighlight( 'textview' );
		showTextView();
	}

	if (target.id.toLowerCase() == 'email'){
		documentCheckoutHighlight( 'email' );
		showEmail();
	}
	...
	if (target.id.toLowerCase() == 'extextview' ){
		...
		textwin = window.open("","Book Maker plain text view","status=1,width=450,height=300");
		textwin.moveTo(0,0);
	...
	if (target.id.toLowerCase() == 'exemail' ){
		...
		mailwin = window.open("","Book Maker mail to a friend","status=1,width=450,height=350");
		mailwin.moveTo(0,0);
		var content = document.createElement('div');
...
function showFullView(){
	...
	var content =  YAHOO.util.Dom.get('documentMakerContent');
	var fullview =  YAHOO.util.Dom.get('fullview');
	docuemtnCheckoutPageClear();

	if (docList.length == 0){
		content.appendChild(document.createTextNode("Your Book Maker is empty."));
		return;
	}
	...
	for (var i in docList){
		...
		img.setAttribute("src", "interfaces/oran/images/book.png");
...
function documentCheckoutPageClear() {
	var bbc = document.getElementById('documentMakerContent');
	if ( bbc == null ) return;
	bbc.innerHTML = '';
}

function documentCheckoutHighlight( id ) {
