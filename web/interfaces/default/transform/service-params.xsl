...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="param[@type='invisible']">
    <xsl:param name="ns">s1.</xsl:param>
    <input type='hidden' name='concat($ns, @name)' value='{@default}'/>
  </xsl:template>
  <!-- boolean params -->
  ...
  <xsl:template match="param[@type='enum_single']">
    ...
    <xsl:choose>
      <xsl:when test="count(option) = 1">
        <xsl:text> </xsl:text><xsl:value-of select="option/displayItem[@name='name']"/>
  ...
  <!-- Don't need to do anything here, handled automatically in the list -->
    ...
    <tr class="queryfieldrow"><xsl:for-each select="param">
      ...
      <td class="queryfieldcell"><xsl:choose>
        <xsl:when test="not(@ignore) or  @ignore != $pos">      
          <xsl:apply-templates select='.'><xsl:with-param name="default" select="$values|$pos"/><xsl:with-param name="ns" select="$ns"/></xsl:apply-templates>
        </xsl:when>
