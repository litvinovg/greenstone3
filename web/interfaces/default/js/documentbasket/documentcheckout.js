...
function navigate(e){
	...
	if (target.id.toLowerCase() == 'extextview' ){
		...
		textwin = window.open("","Document Basket plain text view","status=1,width=450,height=300");
		textwin.moveTo(0,0);
	...
	if (target.id.toLowerCase() == 'exemail' ){
		...
		mailwin = window.open("","Document Basket mail to a friend","status=1,width=450,height=350");
		mailwin.moveTo(0,0);
		var content = document.createElement('div');
...
function showFullView(){
	...
	var content =  YAHOO.util.Dom.get('documentBasketContent');
	...
	for (var i in docList){
		...
		img.setAttribute("src", gs.imageURLs.pageIcon);
...
function documentCheckoutPageClear() {
	var bbc = document.getElementById('documentBasketContent');
	if ( bbc == null ) return;
	bbc.innerHTML = '';
}
